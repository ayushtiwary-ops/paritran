...
## 2026-07-13 (Finale submission: screenshots, publish, QR, deck rebuild)
51. Product screenshots captured live from the running prod stack (:8081) via Playwright, one seed-42 run: discovery (297 -> 6, F1 0.962, 90.8%), casefile (money trail 90.8%, verbatim BNS sections, F9 40/10/0), custody (audit ledger, tamper break mid-chain after real officer link rejections), security (scanners 0 fixable criticals + live egress self-test), evaluation (reproduce diff all green). Every number equals results.json; nothing measured changed.
52. Reproduce had 500'd: the api container's single-file bind mount of results.json (/oracle/results.json) went stale (host inode replaced). `docker compose up -d --force-recreate api web` re-bound it (content sha unchanged, 893fd3...). Operational fix only; engine and results.json untouched.
53. Published via GitHub Pages, not Vercel (push + Pages serves repo, site, PDFs, datasheet, README anchor with zero extra auth; make_qr already targets OWNER.github.io/paritran). Repo github.com/ayushtiwary-ops/paritran (public, default main). Pages from main/docs (+ .nojekyll). Branch renamed master -> main so the /blob/main/ QR links resolve.
54. Demo video hosted on Pages (docs/demo.html + mp4/webm), not YouTube (upload needs the founder's Google auth; not doable unattended). mp4 (H.264) for iOS, webm fallback.
55. Solution document PDF = the rebuilt finale deck PDF (16 pages); no separately-submitted PDF existed on the demo Mac. White paper exported HTML -> PDF (docs/Paritran_White_Paper.pdf).
56. Deck rebuilt: 5 real screenshots on slides 6/14/16, 8 regenerated QR codes on slide 10 (each decodes to its intended URL; every URL 200 or valid mailto). Stale "256" corrected to the measured 297 in build_slides.py and the slide-6 speaker note. PPTX validates (all validations passed); PDF rebuilt from slides.
