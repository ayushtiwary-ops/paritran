...
- `f9_leaked` recomputed non-tautologically. It previously tested `gate(c)` on claims already selected by `not gate(c)`, which is 0 by construction. It now counts ground-truth-labelled fabrications that PASS the gate, which is the meaningful leak definition. Value unchanged at 0, and now meaningful rather than vacuous: a leak is countable under the new definition. In this frozen stub the ten fabrications are all built to fail the verbatim gate, so the leak metric becomes genuinely contingent only on the live model path in the application (SPEC 6.8).
...
**Addendum, same day.** Adversarial review of the milestone diff caught two docstring lines the revision above missed: the module header still called the pipeline "every stage a REAL computation (no placeholders)" and called the stub a "real adversary". Both corrected in one further strings-only touch (the generative step is now labelled a deterministic stub in the header; the gate is described as exercised against known, ground-truth-labelled fabrications). Numeric values re-verified byte-identical after the touch. The freeze applies from this addendum forward.
